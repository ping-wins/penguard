...
| XDR-lite | Endpoint enrollment, inventory, telemetry, VMware pairing discovery, daemon action queue and incident correlation | `apps/xdr_rico` + `apps/agent_private` + `apps/api` gateway | beta | yes | Windows Server lab agent for service validation | `apps/xdr_rico/app/main.py`, `apps/agent_private/README.md`, `docs/superpowers/specs/2026-05-19-xdr-vmware-agent-pairing-discovery.md` | `cd apps/xdr_rico && uv run pytest -q && cd ../agent_private && uv run pytest -q` |
...
| Lab Tools | Synthetic SOC replay and simulator helpers | `apps/api` + lab docs | demo-only | no | yes | `apps/api/app/routers/lab_demo.py`, `docs/mvp/` | `cd apps/api && PENGUARD_ENABLE_LAB_DEMO_TOOLS=true uv run pytest -q tests/test_mvp_demo_chain.py` |
