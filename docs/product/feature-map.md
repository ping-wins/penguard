...
| Integrations | Governed FortiGate traffic-policy orchestration | `apps/api` + `apps/web` | beta | yes | live FortiGate lab required for validation | `apps/api/app/integrations/fortigate/policy_orchestrator.py`, `apps/api/app/routers/integrations.py`, `apps/web/src/components/integrations/LabPolicyWizard.vue`, `docs/architecture/decisions/ADR-2026-05-15-fortigate-policy-orchestration.md` | `cd apps/api && uv run pytest -q tests/test_fortigate_policy_orchestrator.py tests/test_fortigate_policy_requests.py tests/test_fortigate_integrations.py && cd ../web && pnpm test -- integrationsStore sidebarIntegrations widgetRenderers --runInBand` |
