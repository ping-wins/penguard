...
- For allow/log scan validation, a temporary Penguard-owned policy allows
  `10.10.10.10 -> 10.10.20.30` with service `ALL`.
- Penguard API can reach `siem_kowalski`.
...
3. Use Penguard policy orchestration to create or verify a temporary
...
6. Confirm FortiGate Forward Traffic logs show:
   ...
   - matching temporary Penguard-owned policy
   - `logtraffic all`
7. Watch Penguard update through realtime delivery. Use manual/run-now
...
9. If containment is part of the test, approve the SOAR run, review the
   FortiGate policy change, and apply it from Penguard.
...
- Any FortiGate block is applied only after explicit Penguard approval.
...
3. Confirm Penguard policy inventory and FortiGate Forward Traffic logs
   reflect the cleanup.
...
- AI actions fail: configure `PENGUARD_AI_PROVIDER` and
  `PENGUARD_AI_API_KEY`; scripted AI is lab-only.
