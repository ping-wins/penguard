...
- FortiWeb logs WAF traffic/attacks and pushes telemetry to the native
  Penguard endpoint created for the connected FortiWeb integration.
...
Use these object names so screenshots, docs, and Penguard fields line up:
...
- Penguard receives FortiWeb telemetry through
  `/api/soc/ingest/fortiweb/<INTEGRATION_ID>` with the integration telemetry
  token generated by the cockpit.
- `siem_kowalski` creates a WAF incident.
- The dashboard shows an incident toast, recent incident, and ticket without a
  browser refresh.
- SOAR proposes response actions from inside Penguard; live actions still
