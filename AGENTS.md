...
Nota de audit trail no frontend (2026-04-29): a sidebar possui um drawer `Audit Trail` real. Usuários comuns carregam `/api/audit/events`; usuários com role `admin` carregam `/api/admin/audit/events` e veem eventos cross-user. Enquanto o drawer está aberto, `useAuditStore` faz polling a cada 5s para atualizar a trilha sem reload; ao fechar o drawer ou desmontar a sidebar, o polling deve parar. O componente `AuditFeed` deve continuar saneando detalhes sensíveis antes de renderizar e deve exibir estados de loading, erro e vazio.
