  ...
  <div class="h-full w-full flex flex-col gap-4">
    <div class="flex items-start justify-between gap-3">
      <div class="flex min-w-0 items-center gap-3 text-theme-text">
        <Activity :size="24" class="shrink-0 text-red-300" />
        <div class="min-w-0">
          <span class="text-xl font-bold">Anomaly Highlights</span>
          <div class="text-xs text-theme-text-muted truncate">Derived anomalies from system and interface signals</div>
        </div>
      </div>
      <span class="shrink-0 rounded border px-2 py-1 text-xs font-semibold uppercase" :class="highestSeverityClass()">
    ...
    <div class="flex-1 space-y-2 overflow-y-auto pr-1 no-scrollbar">
      ...
      <div v-if="anomalies.length === 0" class="flex h-full min-h-28 flex-col items-center justify-center gap-1 text-center text-xs text-theme-text-muted">
        <span class="font-semibold text-theme-text">No active anomalies</span>
        <span>FortiGate telemetry is not crossing SOC anomaly thresholds.</span>
      </div>
    </div>
  </div>
</template>
