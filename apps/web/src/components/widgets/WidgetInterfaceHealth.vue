...
<template>
  <WidgetShell
    :widget-id="props.catalogId || 'fortigate-interface-health'"
    title="Interface Health"
    subtitle="Operational link state and traffic totals"
    :icon="Network"
    source="fortigate"
    :disable-drill="true"
    :disable-detail="true"
    ...
    <template #glance>
      <div class="flex items-center justify-end text-xs text-theme-text-muted">
        <div class="text-right">
          <span class="font-bold text-emerald-300">{{ summary.up }}</span> up ·
          <span class="font-bold text-red-300">{{ summary.down }}</span> down
        </div>
      </div>
